...
- `crates/raid-defense-core` owns ECS entity/component state, resources, production, spending, theft, game rules, command validation, grid occupancy, pathfinding, deterministic simulation, replay, and checksums.
...
- The Town Hall is the authoritative storage location for settlement resources. Do not add browser/global shadow resource counters.
- Wood is the first resource. Towers, tower upgrades, and Sawmills spend stored Town Hall wood.
- Sawmills produce wood through authoritative `ResourceProducer` components and deposit it into bounded Town Hall `ResourceStorage`.
- Combat kills do not create wood. Production and storage remain the economic authority.
...
- Extend generic resource storage/production components when adding resources or producers rather than introducing unrelated special-case counters.
...
- Prefer components and systems over entity-specific inheritance or monolithic per-entity structs as buildings, towers, raiders, projectiles, effects, resources, and units grow.
...
- Prefer integer or fixed-point representations for authoritative movement, economy, and combat rules.
...
- Browser path previews, range displays, and economy projections are advisory presentation; Rust remains authoritative for legality, production, spending, theft, movement, and combat.
...
- Keep economy/build/raid controls and feedback close to the world state they affect.
